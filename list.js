import { readRequest, formatResponseError, STICKERS_PER_COUNTRY, COUNTRY_IDS, getSortInversePermutation, stickerName } from "./common.js?v2";
...
readRequest(publicID).then(async response => {
	...
	function updateList() {
		...
		for (let c = 0; c < COUNTRY_IDS.length; c++) {
			...
			teamContainer.append(COUNTRY_IDS[country], ': ');
			let first = true;
			for (let s = 0; s < STICKERS_PER_COUNTRY; s++) {
				...
				if (n > 2) {
					...
					teamContainer.append(dupCount);
				}
			}
			if (!first) {
				listContainer.append(teamContainer);
			}
		}
	}
